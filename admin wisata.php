...
$query_wisata = "SELECT id_wisata, nama, deskripsi, gambar FROM wisata ORDER BY nama ASC";
$result_wisata = mysqli_query($conn, $query_wisata);

if (!$result_wisata) {
    die("Error mengambil data wisata: " . mysqli_error($conn));
}
?>
    ...
    <div class="main-content">
      ...
      <div class="content">
        ...
        <div class="actions-section">
          <a href="add wisata.php" class="add-btn">
            <i class="fas fa-plus"></i>
            Tambah Wisata Baru
          </a>
          <div class="stats-info">
            <i class="fas fa-info-circle"></i>
            Total: <?= mysqli_num_rows($result_wisata) ?> destinasi wisata
          </div>
        </div>
          ...
          <table class="table">
            <thead>
              <tr>
                ...
                <th style="width: 200px;"><i class="fas fa-mountain"></i> NAMA WISATA</th>
              ...
              if (mysqli_num_rows($result_wisata) > 0) {
                  while ($row = mysqli_fetch_assoc($result_wisata)) {
                      ...
                      if (!empty($row['gambar'])) {
                          ...
                          $gambar_display = '<img src="uploads/' . htmlspecialchars($gambarPertama) . '" alt="Gambar Wisata" class="img-thumbnail-custom">';
                      } else {
                          $gambar_display = '<div class="text-muted"><i class="fas fa-image"></i><br>Tidak ada gambar</div>';
                      }
                      ?>
                      <tr>
                          <td><?= $no++ ?></td>
                          <td>
                              <div class="wisata-name"><?= htmlspecialchars($row['nama']) ?></div>
                          </td>
                          <td>
                          ...
                          <td>
                              <div class="action-btn-wrapper">
                                <a href="edit wisata.php?id=<?= htmlspecialchars($row['id_wisata']) ?>" class="action-btn edit-btn">
                                  <i class="fas fa-edit"></i> Edit
                                </a>
                                <a href="delete wisata.php?id=<?= htmlspecialchars($row['id_wisata']) ?>" class="action-btn delete-btn" onclick="return confirm('Apakah Anda yakin ingin menghapus wisata ini?')">
                  ...
                  echo '<tr><td colspan="5" class="empty-state">
                            <div>
                                ...
                                <p>Mulai tambahkan destinasi wisata pertama Anda dengan mengklik tombol "Tambah Wisata Baru" di atas.</p>
                            </div>
                        </td></tr>';
              }
              ?>
